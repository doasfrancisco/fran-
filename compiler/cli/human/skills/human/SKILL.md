...
- an expansion without words, a decompile, a create → nothing is mapped at the click; the event alone is queued, and claude maps what it writes.
...
- `decompile`: the file has no map; write its whole-file telling — step 4, the rail — and `human map <name>` it.
...
- `expand` without `new_text`: write the expansion of the highlighted part of `target` yourself — the flow or a tighter rail on what those words tell — pinned into the code and the file entries, never into `target`, and `human map <name>` it. Then the same plain retext of `target`: the highlighted words pinned into an anchor of the new entry. A pin from the expansion into `target` would make it a top over `target` and close the circle for that last step.
- `create`: write a plainer telling over `target`, its heads pinned with `e<target>:` into the anchors of `target`, and `human map <name>` it; it stands below `target` in the reader.
